...
from transformers import AutoTokenizer, MBartTokenizer, PreTrainedTokenizer
from tqdm import tqdm
import penman

from amr_bart.data.new_linearization import Linearizer, unescape_xml, escape_xml
...
def decode_and_escape(
    ...
    filtered_tokens = [token if token in tokens_to_add else escape_xml(token) for token in filtered_tokens]
...
if __name__ == '__main__':
    # We have to use the slow tokenizer for decoding because otherwise decode_and_escape will not work
    # Maybe use fast tokenizer for encoding, slow for decoding?
    tokenizer = AutoTokenizer.from_pretrained("facebook/mbart-large-cc25", use_fast=False)
    init_max_voc = len(tokenizer)
    tokenizer.add_tokens(list(tokens_to_add))
    print(f"Added {len(tokens_to_add):,} new tokens!")


    pdin = Path(r"D:\corpora\amr_annotation_3.0\data\amrs_fixed\split")
    for pfin in tqdm(list(pdin.rglob("*.txt")), unit="file"):
        with pfin.open(encoding="utf-8") as fhin:
            for tree in penman.iterparse(fhin):
                ...
                decoded = decode_and_escape(encoded, tokenizer, skip_special_tokens=True)

                try:
                    linearizer_re = Linearizer.from_linearized(decoded)
                except Exception as exc:
                    print(decoded)
                    raise exc
                linearizer_re.penman_tree.reset_variables()

                if linearizer.penman_tree != linearizer_re.penman_tree:
                    # Still getting problems here: polarity is missing in the output
                    # Maybe because it is already in the vocabulary? <negation/>
                    print(linearizer.penman_tree)
                    print(linearizer_re.penman_tree)
                    exit()
