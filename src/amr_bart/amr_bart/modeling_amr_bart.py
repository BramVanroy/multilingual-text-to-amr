...
class AMRBartForConditionalGeneration(PretrainedBartModel):
    ...
    def _generate_beam_search(
        ...
        generated_hyps = [
            BeamHypotheses(num_beams, length_penalty, early_stopping=early_stopping) for _ in range(batch_size)
        ]

        # scores for each sentence in the beam
        beam_scores = torch.zeros((batch_size, num_beams), dtype=torch.float, device=input_ids.device)

        # for greedy decoding it is made sure that only tokens of the first beam are considered to avoid sampling
        # the exact same tokens three times
        if do_sample is False:
            beam_scores[:, 1:] = -1e9
