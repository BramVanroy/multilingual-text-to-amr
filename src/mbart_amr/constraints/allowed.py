...
class AllowedTokensProcessor(AMRLogitsProcessorBase):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for beam_idx in range(input_ids.size(0)):
            ...
            if not (last_item == self.start_rel_idx or ends_in_role(inputs, self.tokenizer)):
                mask = self.ref_idxs
                # ... except for :ref1 which can occur at the start of the sequence
                if num_inputs == 1:
                    mask = mask[~torch.isin(mask, self.ref_idxs[0])]
            ...
            if last_item in self.special_tokens_idxs or last_item in self.added_tokens_idxs:
